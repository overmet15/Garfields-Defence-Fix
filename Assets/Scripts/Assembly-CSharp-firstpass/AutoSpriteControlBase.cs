...
public abstract class AutoSpriteControlBase : AutoSpriteBase, IEZDragDrop, IControl, IPackableControl, IUIObject, ISpritePackable
{
	...
	public override void Hide(bool tf)
	{
		...
		else if (IsHidden() && !tf && GetComponent<Collider>() is BoxCollider)
		{
	...
	public virtual void Copy(SpriteRoot s, ControlCopyFlags flags)
	{
		...
		if ((flags & ControlCopyFlags.Text) == ControlCopyFlags.Text)
		{
			...
			{
				GameObject gameObject = UnityEngine.Object.Instantiate(autoSpriteControlBase.spriteText.gameObject);
				gameObject.transform.parent = base.transform;
		...
		if ((flags & ControlCopyFlags.Appearance) == ControlCopyFlags.Appearance)
		{
			...
			{
				if (GetComponent<Collider>().GetType() == autoSpriteControlBase.GetComponent<Collider>().GetType())
				{
					...
					else if (autoSpriteControlBase.GetComponent<Collider>() is MeshCollider)
					{
						...
						MeshCollider meshCollider = (MeshCollider)GetComponent<Collider>();
					...
					if (GetComponent<Collider>() != null)
					{
						GetComponent<Collider>().isTrigger = autoSpriteControlBase.GetComponent<Collider>().isTrigger;
					}
				}
			}
			else if (Application.isPlaying)
			{
				if (GetComponent<Collider>() == null && width != 0f && height != 0f && !float.IsNaN(width) && !float.IsNaN(height))
				{
				...
				{
					BoxCollider boxCollider4 = (BoxCollider)GetComponent<Collider>();
				...
				{
					SphereCollider sphereCollider3 = (SphereCollider)GetComponent<Collider>();
	...
	{
		if (deleted || m_spriteMesh == null || !(GetComponent<Collider>() is BoxCollider) || IsHidden() || m_spriteMesh == null || customCollider)
		{
			return;
		}
