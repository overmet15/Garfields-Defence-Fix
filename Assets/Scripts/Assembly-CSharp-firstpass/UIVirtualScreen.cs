...
public class UIVirtualScreen : MonoBehaviour, IEZDragDrop, IUIObject
{
	...
	protected void InputProcessor(ref POINTER_INFO ptr)
	{
		shuttingDown = false;
		StopAllCoroutines();
		ptr.devicePos = new Vector3(ptr.hitInfo.textureCoord.x * (float)screenCamera.pixelWidth, ptr.hitInfo.textureCoord.y * (float)screenCamera.pixelHeight, 0f);
		Vector3 vector = ptr.devicePos;
		float z = ptr.inputDelta.z;
		RaycastHit hitInfo;
		if (ptr.prevRay.direction.sqrMagnitude > 0f && GetComponent<Collider>().Raycast(ptr.prevRay, out hitInfo, ptr.rayDepth))
		{
			vector = new Vector3(hitInfo.textureCoord.x * (float)screenCamera.pixelWidth, hitInfo.textureCoord.y * (float)screenCamera.pixelHeight, 0f);
	...
	{
		if (!(screenCamera == null) && !screenCamera.gameObject.activeInHierarchy)
		{
	...
	{
		Vector2 vector = new Vector2(ptr.hitInfo.textureCoord.x * (float)screenCamera.pixelWidth, ptr.hitInfo.textureCoord.y * (float)screenCamera.pixelHeight);
		IUIObject iUIObject = null;
		bool flag = !screenCamera.gameObject.activeInHierarchy;
		if (flag)
		{
			screenCamera.gameObject.SetActive(true);
		}
