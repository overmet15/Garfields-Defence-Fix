...
public class UIScrollList : MonoBehaviour, IEZDragDrop, IUIObject
{
	...
	public void InsertItem(IUIListObject item, int position, string text, bool doEasing)
	{
		...
		if (activateWhenAdding && !((Component)item).gameObject.activeInHierarchy)
		{
			((Component)item).gameObject.SetActiveRecursivelyLegacy(true);
		}
		if (!base.gameObject.activeInHierarchy)
		{
			((Component)item).gameObject.SetActiveRecursivelyLegacy(false);
		}
		...
		if (clipContents)
		{
			...
			if (!item.Managed)
			{
				item.gameObject.SetActiveRecursivelyLegacy(false);
			}
		}
		item.Index = position;
		newItems.Add(item);
	...
	{
		if (mover == null || items.Count < 1 || !clipContents || !base.gameObject.activeInHierarchy)
		{
			return;
		}
		IUIListObject iUIListObject = null;
		...
		if (orientation == ORIENTATION.HORIZONTAL)
		{
			...
			if (direction == DIRECTION.TtoB_LtoR)
			{
				...
				if (iUIListObject != null)
				{
					...
					{
						iUIListObject.gameObject.SetActiveRecursivelyLegacy(true);
					}
					...
					if (iUIListObject.BottomRightEdge.x + x2 < num2)
					{
						for (i = iUIListObject.Index + 1; i < items.Count; i++)
						{
							...
							if (items[i].BottomRightEdge.x + x2 >= num2)
							{
								...
								{
									items[i].gameObject.SetActiveRecursivelyLegacy(true);
								}
								items[i].Hide(false);
				...
				if (iUIListObject != null)
				{
					...
					if (iUIListObject.BottomRightEdge.y + y2 > num4)
					{
						for (j = iUIListObject.Index + 1; j < items.Count; j++)
						{
							...
							{
								if (!items[j].gameObject.activeInHierarchy)
								{
									items[j].gameObject.SetActiveRecursivelyLegacy(true);
								}
								items[j].Hide(false);
		...
		if (visibleItems.Count > 0)
		{
			if (visibleItems[0].Index > iUIListObject2.Index || visibleItems[visibleItems.Count - 1].Index < iUIListObject.Index)
			{
				for (int k = 0; k < visibleItems.Count; k++)
				{
					...
					{
						visibleItems[k].gameObject.SetActiveRecursivelyLegacy(false);
					}
				}
			}
			else
			{
				for (int l = 0; l < visibleItems.Count && visibleItems[l].Index < iUIListObject.Index; l++)
				{
					...
					{
						visibleItems[l].gameObject.SetActiveRecursivelyLegacy(false);
					}
				}
				...
				while (num5 > -1 && visibleItems[num5].Index > iUIListObject2.Index)
				{
					...
					{
						visibleItems[num5].gameObject.SetActiveRecursivelyLegacy(false);
					}
					num5--;
				}
			}
	...
	{
		base.gameObject.SetActiveRecursivelyLegacy(true);
	...
	public void RemoveItem(int index, bool destroy, bool doEasing)
	{
		if (index >= 0 && index < items.Count)
		{
			...
			else
			{
				...
				items[index].gameObject.SetActiveRecursivelyLegacy(false);
			}
	...
	public void ClearList(bool destroy)
	{
		...
		for (int i = 0; i < items.Count; i++)
		{
			...
			{
				items[i].gameObject.SetActiveRecursivelyLegacy(false);
			}
		}
		visibleItems.Clear();
		items.Clear();
