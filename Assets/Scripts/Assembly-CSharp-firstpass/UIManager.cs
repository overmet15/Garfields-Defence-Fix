...
public class UIManager : MonoBehaviour
{
	...
	protected void DispatchInput()
	{
		...
		if (mouseTouchListeners != null)
		{
			for (int j = 0; j < numActivePointers; j++)
			{
				for (int k = 0; k < uiCameras.Length; k++)
				{
					if (uiCameras[k].camera.gameObject.activeInHierarchy)
					{
		...
		else
		{
			for (int l = 0; l < numActivePointers; l++)
			{
				for (int m = 0; m < uiCameras.Length; m++)
				{
					if (uiCameras[m].camera.gameObject.activeInHierarchy)
					{
	...
	protected void PollMouse()
	{
		...
		for (int i = 1; i < uiCameras.Length; i++)
		{
			...
			else if (uiCameras[i].camera.gameObject.activeInHierarchy)
			{
	...
	{
		numActivePointers = Mathf.Min(numTouches, (Application.isMobilePlatform) ? Input.touchCount : InputHelper.GetTouches().Count);
		for (int i = 0; (Application.isMobilePlatform) ? i < numActivePointers : i < InputHelper.GetTouches().Count; i++)
		{
			...
			if (!Application.isMobilePlatform)
			{
				Vector2 v2 = touch.position;
				touch.deltaPosition = lastTouchPos;
				lastTouchPos = v2;
			}
	...
	public void Detarget(int pointerID)
	{
		...
		for (int i = 0; i < uiCameras.Length; i++)
		{
			if (uiCameras[i].camera != null && uiCameras[i].camera.gameObject.activeInHierarchy && pointers[i, pointerID].targetObj != null)
			{
	...
	public void DetargetAllExcept(int pointerID)
	{
		for (int i = 0; i < numActivePointers; i++)
		{
			...
			{
				if (uiCameras[j].camera != null && uiCameras[j].camera.gameObject.activeInHierarchy && pointers[j, activePointers[i]].targetObj != null)
				{
					POINTER_INFO ptr = default(POINTER_INFO);
	...
	public void Retarget(IUIObject oldObj, IUIObject newObj)
	{
		...
		for (int i = 0; i < numActivePointers; i++)
		{
			for (int j = 0; j < uiCameras.Length; j++)
			{
				if (uiCameras[j].camera != null && uiCameras[j].camera.gameObject.activeInHierarchy && pointers[j, activePointers[i]].targetObj != null && pointers[j, activePointers[i]].targetObj == oldObj)
				{
